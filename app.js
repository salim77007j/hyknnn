...
async function loadImages(page = 1, reset = true) {
    try {
        ...
        // جلب التصنيفات منفصلاً لتجنب مشاكل العلاقات
        ...
        if (categoryIds.length > 0) {
            try {
                ...
                if (catsData) {
                    categories = Object.fromEntries(catsData.map(cat => [cat.id, cat]));
                }
            } catch (catError) {
                console.warn('تعذر تحميل التصنيفات:', catError.message);
            }
        }
        ...
        const enrichedImages = images.map(image => ({
            ...image,
            categories: categories[image.category_id] || null
        }));

        if (reset) {
            currentImages = enrichedImages;
        } else {
            currentImages = [...currentImages, ...enrichedImages];
        }

        // تحديث عدد الصور الجديدة للزر
...
async function openImageModal(imageId) {
    try {
        // زيادة عدد المشاهدات (تجاهل الأخطاء)
        try {
            await supabase.rpc('increment_view_count', { image_uuid: imageId });
        } catch (viewError) {
            console.warn('تعذر زيادة عدد المشاهدات:', viewError.message);
        }

        // جلب تفاصيل الصورة بطريقة آمنة
        ...
        // تحديث المودال
        document.getElementById('modal-image').src = getImageUrl(enrichedImage.file_path);
        document.getElementById('modal-title').textContent = enrichedImage.title;
        document.getElementById('modal-description').textContent = enrichedImage.description || 'لا يوجد وصف';
        document.getElementById('modal-views').textContent = enrichedImage.view_count || 0;
        document.getElementById('modal-downloads').textContent = enrichedImage.download_count || 0;
        document.getElementById('modal-date').textContent = new Date(enrichedImage.created_at).toLocaleDateString('ar');
        ...
        document.getElementById('download-btn').onclick = () => downloadImage(enrichedImage);
        document.getElementById('share-btn').onclick = () => shareImage(enrichedImage);
